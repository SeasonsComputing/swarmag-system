...
The domain model is implemented as a TypeScript library under `source/domain`, with shared primitives in `source/utils`. It is the authoritative source of meaning for the system.
...
`architecture.md` documents how the system is organized to support this domain model; it does not define the domain itself.
...
API handler conventions, validation rules, and transport concerns are documented in `docs/foundation/architecture.md`.
