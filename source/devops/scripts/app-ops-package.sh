...
if [[ "${VITE_PACKAGE_APP_ID:-}" != "swarmag-app-ops" ]]; then
  echo "Invalid VITE_PACKAGE_APP_ID='${VITE_PACKAGE_APP_ID:-}'. Expected 'swarmag-app-ops'"
  exit 1
fi
...
for file in "${REQUIRED_STATIC_FILES[@]}"; do
  ...
  if [[ ! -f "${SOURCE_FILE}" ]]; then
    echo "Missing required app-ops static file: ${SOURCE_FILE}"
    exit 1
  fi
...
ARTIFACT_BASENAME="swarmag-app-ops-${TARGET}-${TIMESTAMP}-${GIT_SHA}"
