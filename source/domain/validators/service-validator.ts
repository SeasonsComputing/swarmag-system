...
import type { ServiceCreate, ServiceRequiredAssetTypeCreate, ServiceUpdate } from '@domain/protocols/service-protocol.ts'
...
export const validateServiceCreate = (input: ServiceCreate): string | null => {
  ...
  if (!SERVICE_CATEGORIES.includes(input.category as ServiceCategory)) return 'category must be a valid ServiceCategory'
  return null
}
...
export const validateServiceUpdate = (input: ServiceUpdate): string | null => {
  ...
  if (input.category !== undefined && !SERVICE_CATEGORIES.includes(input.category as ServiceCategory)) return 'category must be a valid ServiceCategory'
  return null
}

/** Validates ServiceRequiredAssetTypeCreate; returns error message or null. */
export const validateServiceRequiredAssetTypeCreate = (input: ServiceRequiredAssetTypeCreate): string | null => {
