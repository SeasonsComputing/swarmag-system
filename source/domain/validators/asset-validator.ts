...
import type { AssetCreate, AssetTypeCreate, AssetTypeUpdate, AssetUpdate } from '@domain/protocols/asset-protocol.ts'
...
export const validateAssetTypeUpdate = (input: AssetTypeUpdate): string | null => {
  ...
  if (input.active !== undefined && typeof input.active !== 'boolean') return 'active must be a boolean'
  return null
}
...
export const validateAssetCreate = (input: AssetCreate): string | null => {
  ...
  if (!ASSET_STATUSES.includes(input.status as AssetStatus)) return 'status must be a valid AssetStatus'
  return null
}
...
export const validateAssetUpdate = (input: AssetUpdate): string | null => {
  ...
  if (input.status !== undefined && !ASSET_STATUSES.includes(input.status as AssetStatus)) return 'status must be a valid AssetStatus'
  return null
}
