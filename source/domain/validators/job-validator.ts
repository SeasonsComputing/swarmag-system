...
export const validateJobUpdate = (input: JobUpdate): string | null => {
  ...
  if (input.status !== undefined && !JOB_STATUSES.includes(input.status as JobStatus)) return 'status must be a valid JobStatus'
  return null
}

/** Validates JobAssessmentCreate; returns error message or null. */
export const validateJobAssessmentCreate = (input: JobAssessmentCreate): string | null => {
  if (!isId(input.jobId)) return 'jobId must be a valid Id'
  if (!isId(input.assessorId)) return 'assessorId must be a valid Id'
  if (!isCompositionPositive(input.locations, isLocation)) return 'locations must be a non-empty array of valid locations'
  return null
}

/** Validates JobAssessmentUpdate; returns error message or null. */
export const validateJobAssessmentUpdate = (input: JobAssessmentUpdate): string | null => {
  if (!isId(input.id)) return 'id must be a valid Id'
  if (input.assessorId !== undefined && !isId(input.assessorId)) return 'assessorId must be a valid Id'
  if (input.locations !== undefined && !isCompositionPositive(input.locations, isLocation)) return 'locations must be a non-empty array of valid locations'
  return null
}
...
export const validateJobWorkflowCreate = (input: JobWorkflowCreate): string | null => {
  if (!isId(input.jobId)) return 'jobId must be a valid Id'
  if (typeof input.sequence !== 'number' || input.sequence < 0) return 'sequence must be a non-negative number'
...
export const validateJobPlanUpdate = (input: JobPlanUpdate): string | null => {
  if (!isId(input.id)) return 'id must be a valid Id'
  if (input.scheduledStart !== undefined && !isWhen(input.scheduledStart)) return 'scheduledStart must be a valid When'
  if (input.scheduledEnd !== undefined && !isWhen(input.scheduledEnd)) return 'scheduledEnd must be a valid When'
  return null
}

/** Validates JobPlanAssignmentCreate; returns error message or null. */
export const validateJobPlanAssignmentCreate = (input: JobPlanAssignmentCreate): string | null => {
...
export const validateJobPlanAssignmentUpdate = (input: JobPlanAssignmentUpdate): string | null => {
  if (!isId(input.id)) return 'id must be a valid Id'
  if (input.role !== undefined && !USER_ROLES.includes(input.role as UserRole)) return 'role must be a valid UserRole'
  return null
}

/** Validates JobPlanChemicalCreate; returns error message or null. */
export const validateJobPlanChemicalCreate = (input: JobPlanChemicalCreate): string | null => {
  if (!isId(input.planId)) return 'planId must be a valid Id'
  if (!isId(input.chemicalId)) return 'chemicalId must be a valid Id'
  if (typeof input.amount !== 'number' || input.amount <= 0) return 'amount must be a positive number'
  if (!CHEMICAL_UNITS.includes(input.unit as ChemicalUnit)) return 'unit must be a valid ChemicalUnit'
  return null
}

/** Validates JobPlanChemicalUpdate; returns error message or null. */
export const validateJobPlanChemicalUpdate = (input: JobPlanChemicalUpdate): string | null => {
  if (!isId(input.id)) return 'id must be a valid Id'
  if (input.amount !== undefined && (typeof input.amount !== 'number' || input.amount <= 0)) return 'amount must be a positive number'
  if (input.unit !== undefined && !CHEMICAL_UNITS.includes(input.unit as ChemicalUnit)) return 'unit must be a valid ChemicalUnit'
  return null
}
...
export const validateJobWorkCreate = (input: JobWorkCreate): string | null => {
  if (!isId(input.jobId)) return 'jobId must be a valid Id'
  if (!isCompositionPositive(input.work, isId)) return 'work must be a non-empty array of valid Ids'
...
export const validateJobWorkLogEntryCreate = (input: JobWorkLogEntryCreate): string | null => {
  if (!isId(input.jobId)) return 'jobId must be a valid Id'
  if (!isId(input.userId)) return 'userId must be a valid Id'
