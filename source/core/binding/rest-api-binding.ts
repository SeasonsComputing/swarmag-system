...
function endpoint(path: string): string {
  // TODO: critical!!!!!
  const baseURL = "https://127.0.0.1"; //Config.get('VITE_API_URL')
  return `${baseURL}${path}`
}
...
async function unwrapList<T>(
  ...
  if (!response.ok) {
    throw new ApiError((body.error as string) ?? fallbackError, response.status,
      body.details as string | undefined)
  }
 ...
 */
export function makeApiClient<T, TCreate, TUpdate>(
  spec: ApiSpecification<T, TCreate, TUpdate>
) {
  const { basePath } = spec

  return {
    async create(input: TCreate): Promise<T> {
      const res = await fetch(endpoint(`${basePath}/create`), {
        method: 'POST',
    ...
    async update(input: TUpdate): Promise<T> {
      const res = await fetch(endpoint(`${basePath}/update`), {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(input)
      })
    ...
    async delete(id: ID): Promise<DeleteResult> {
      const res = await fetch(endpoint(`${basePath}/delete`), {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
    ...
    async list(options?: ListOptions): Promise<ListResult<T>> {
      const res = await fetch(endpoint(`${basePath}/list`), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
