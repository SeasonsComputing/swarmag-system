...
export const workflowToRow = (workflow: Workflow) => ({ id: workflow.id, service_id: workflow.serviceId,
  name: workflow.name, description: workflow.description ?? null, version: workflow.version,
  effective_from: workflow.effectiveFrom, steps: workflow.steps, locations_required: workflow.locationsRequired ?? null,
  created_at: workflow.createdAt, updated_at: workflow.updatedAt, payload: workflow })
...
export const rowToWorkflow = (row: unknown): Workflow => {
  ...
  if (!id || !serviceId || !name || typeof version !== 'number' || !effectiveFrom || !Array.isArray(steps)) {
    throw new Error('Workflow row is missing required fields')
  }

  return { id, serviceId, name, description: (record.description ?? undefined) as string | undefined, version,
    effectiveFrom, steps,
    locationsRequired: Array.isArray(record.locations_required ?? record.locationsRequired)
      ...
      : undefined, createdAt: (record.created_at ?? record.createdAt) as string,
