 ...
 */
const handle = async (req: ApiRequest<CustomerCreateInput>): Promise<ApiResponse> => {
  ...
  if (error) return toInternalError('Failed to create customer', error)

  return toCreated(customer)
}
