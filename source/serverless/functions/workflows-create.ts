 ...
 */
const handle = async (
  req: ApiRequest<WorkflowCreateInput>
): Promise<ApiResponse> => {
  if (req.method !== 'POST') {
  ...
  const { error } = await Supabase.client()
    .from('workflows')
    .insert(workflowToRow(workflow))

  if (error) {
    return {
      statusCode: HttpCodes.internalError,
      body: { error: 'Failed to create workflow', details: error.message }
    }
  }

  return { statusCode: HttpCodes.created, body: { data: workflow } }
}
