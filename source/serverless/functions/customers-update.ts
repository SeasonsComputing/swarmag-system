...
import { type CustomerUpdateInput, validateCustomerUpdate } from '@domain/validators/customer-validators.ts'
import {
  type ApiRequest,
  type ApiResponse,
  toInternalError,
...
const handle = async (req: ApiRequest<CustomerUpdateInput>): Promise<ApiResponse> => {
  ...
  const { data: existingRow, error: fetchError } = await supabase.from('customers').select('*').eq('id', req.body.id)
    .single()
  ...
  const updated: Customer = { ...current, name: req.body.name?.trim() ?? current.name,
    status: req.body.status ?? current.status, line1: req.body.line1?.trim() ?? current.line1,
    line2: req.body.line2 === null ? undefined : req.body.line2?.trim() ?? current.line2,
    city: req.body.city?.trim() ?? current.city, state: req.body.state?.trim() ?? current.state,
    postalCode: req.body.postalCode?.trim() ?? current.postalCode, country: req.body.country?.trim() ?? current.country,
    accountManagerId: req.body.accountManagerId === null
      ? undefined
      : req.body.accountManagerId ?? current.accountManagerId,
    primaryContactId: req.body.primaryContactId ?? current.primaryContactId, updatedAt: when() }

  const { error: updateError } = await supabase.from('customers').update(customerToRow(updated)).eq('id', updated.id)
