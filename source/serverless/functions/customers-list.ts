...
const handle = async (req: ApiRequest<undefined, ListQuery>): Promise<ApiResponse> => {
  ...
  const { data, error, count } = await supabase.from('customers').select('*', { count: 'exact' }).range(cursor,
    rangeEnd)

  if (error) return toInternalError('Failed to load customers', error)
  ...
  return { statusCode: HttpCodes.ok, body: { data: customers, cursor: nextCursor, hasMore } }
}
