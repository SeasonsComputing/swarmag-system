...
Canonical `Question` instances with `type = 'internal'` used for system-generated telemetry and operational log entries. These are seed data and are referenced by `JobWorkLogEntry.answer.questionId`. The Key is the `Question.prompt` value — for internal questions it serves as both the display value and machine-readable identifier.
